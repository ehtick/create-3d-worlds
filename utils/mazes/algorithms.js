...
function divide(grid, row, column, height, width) {
  if (height <= 1 || width <= 1 || height < 5 && width < 5 && Math.random() < 0.25) return

  if (height > width)
    divide_horizontally(grid, row, column, height, width)
  else
    divide_vertically(grid, row, column, height, width)
}

function divide_horizontally(grid, row, column, height, width) {
  ...
  for (let x = 0; x < width; x += 1) {
    ...
    const cell = grid.cell(row + divide_south_of, column + x)
    cell.unlink(cell.south)
  }

  divide(grid, row, column, divide_south_of + 1, width)
  divide(grid, row + divide_south_of + 1, column, height - divide_south_of - 1, width)
}

function divide_vertically(grid, row, column, height, width) {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    const cell = grid.cell(row + y, column + divide_east_of)
    cell.unlink(cell.east)
  }

  divide(grid, row, column, height, divide_east_of + 1)
  divide(grid, row, column + divide_east_of + 1, height, width - divide_east_of - 1)
}
