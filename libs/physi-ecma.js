...
Physijs.HingeConstraint.prototype.setLimits = function(low, high, bias_factor, relaxation_factor) {
  this.scene.execute('hinge_setLimits', { constraint: this.id, low, high, bias_factor, relaxation_factor })
}
Physijs.HingeConstraint.prototype.enableAngularMotor = function(velocity, acceleration) {
  this.scene.execute('hinge_enableAngularMotor', { constraint: this.id, velocity, acceleration })
}
...
Physijs.SliderConstraint.prototype.setLimits = function(lin_lower, lin_upper, ang_lower, ang_upper) {
  this.scene.execute('slider_setLimits', { constraint: this.id, lin_lower, lin_upper, ang_lower, ang_upper })
}
...
Physijs.SliderConstraint.prototype.enableLinearMotor = function(velocity, acceleration) {
  this.scene.execute('slider_enableLinearMotor', { constraint: this.id, velocity, acceleration })
}
...
Physijs.SliderConstraint.prototype.enableAngularMotor = function(velocity, acceleration) {
  this.scene.execute('slider_enableAngularMotor', { constraint: this.id, velocity, acceleration })
}
...
Physijs.DOFConstraint.prototype.configureAngularMotor = function(which, low_angle, high_angle, velocity, max_force) {
  this.scene.execute('dof_configureAngularMotor', { constraint: this.id, which, low_angle, high_angle, velocity, max_force })
}
...
Physijs.Scene.prototype._updateCollisions = function(data) {
  ...
  let i, j, offset, object, object2, id1, id2,
...
Physijs.Scene.prototype.add = function(object) {
  ...
  if (object._physijs) {
    ...
    if (object instanceof Physijs.Vehicle) {
      ...
      // Check for scaling
      const mass_scaling = new THREE.Vector3(1, 1, 1)
...
Physijs.Mesh.prototype.__defineSetter__('mass', function(mass) {
  ...
  if (this.world)
    this.world.execute('updateMass', { id: this._physijs.id, mass })
...
Physijs.Mesh.prototype.setDamping = function(linear, angular) {
  if (this.world)
    this.world.execute('setDamping', { id: this._physijs.id, linear, angular })
...
Physijs.Mesh.prototype.setCcdMotionThreshold = function(threshold) {
  if (this.world)
    this.world.execute('setCcdMotionThreshold', { id: this._physijs.id, threshold })
...
Physijs.Mesh.prototype.setCcdSweptSphereRadius = function(radius) {
  if (this.world)
    this.world.execute('setCcdSweptSphereRadius', { id: this._physijs.id, radius })
