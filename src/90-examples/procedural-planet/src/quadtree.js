...
export const quadtree = (function() {
  ...
  class CubeQuadTree {
    ...
    BuildRootNeighbourInfo_() {
      const _FindClosestNeighbour = (edgeMidpoint, otherNodes) => {
        const neighbours = [...otherNodes].sort((a, b) => a.sphereCenter.distanceTo(edgeMidpoint) - b.sphereCenter.distanceTo(edgeMidpoint))
        const test = [...otherNodes].map(c => c.sphereCenter.distanceTo(edgeMidpoint)).sort((a, b) => a - b)
        return neighbours[0]
      }
  ...
  class Node {
    ...
    GetClosestChild(node) {
      const children = [...this.children].sort((a, b) => a.sphereCenter.distanceTo(node.sphereCenter) - b.sphereCenter.distanceTo(node.sphereCenter))
      const test = [...this.children].map(c => c.sphereCenter.distanceTo(node.sphereCenter)).sort((a, b) => a - b)
      return children[0]
    }
