...
class _TerrainBuilderThreadedWorker {
  ...
  _ComputeNormal_CentralDifference(xp, yp, stepSize) {
    ...
    const { radius } = this.params_
    const { offset } = this.params_
    const { width } = this.params_
    const half = width / 2
    const resolution = this.params_.resolution + 2
